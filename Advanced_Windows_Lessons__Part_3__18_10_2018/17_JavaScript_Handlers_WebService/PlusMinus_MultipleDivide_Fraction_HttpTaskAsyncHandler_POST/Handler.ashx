...
public class Handler : HttpTaskAsyncHandler
{
    ...
    {
        if (context.Request["JsonString_0"] == null
    ||
    context.Request["JsonString_1"] == null
    ||
    context.Request["cmd"] == null)
            return;

        JavaScriptSerializer myJavaScriptSerializer = new JavaScriptSerializer();
        Fraction A = (Fraction)myJavaScriptSerializer.Deserialize(context.Request["JsonString_0"], typeof(Fraction));
        Fraction B = (Fraction)myJavaScriptSerializer.Deserialize(context.Request["JsonString_1"], typeof(Fraction));

        if (context.Request["cmd"] == "PlusMinus")
